...
void Reset_GC3355(void)
{
	uint32_t i;
	 ...
	 RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA , ENABLE);  //RCC_APB2Periph_AFIO
       GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;                 //选择引脚5
...
void USART_To_USB_Send_Data(USART_TypeDef* USARTx)
{
  ...
  {
    USART_Rx_Buffer[USART_Rx_ptr_in] = USART_ReceiveData(USARTx) & 0x7F;
  }
  else if (linecoding.datatype == 8)
  {
    USART_Rx_Buffer[USART_Rx_ptr_in] = USART_ReceiveData(USARTx);
  }

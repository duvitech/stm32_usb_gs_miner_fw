...
#ifdef FIXED_INIT_NONCE
static int trans_init_btc_nonce(void)
{
	//swz0207-static int init_nonce = 0;
	int sub;
	u8 nonce[8] = {0x55,0xaa,0x00,0x00,0x00,0x00,0x00,0x00};
	u32 step = 0xffffffff / Max_Chip_Num;
	u32 base = 0;
	
	//swz0207-if (init_nonce != 0) return;
	//swz0207-init_nonce = 1;
	
	for(sub=0;sub<Max_Chip_Num;sub++)
	{
		nonce[2]=(BTC|sub);
		* (u32 *) (nonce+4) = base + sub* step;
		delay_us(CMD_WAIT_TIME);
		if(sub != 0xf)
			trans_btc_cmd(nonce, 8);
		/* for the chip 15, because of the addr = 0xf, need special process */
#if defined(USE_STM3210E_EVAL)			
		else {
			nonce[2]=(BTC|0x7);
			trans_buf_to_uart(BTC_COM2, nonce, 8);
		}
#endif
	}
}
...
static void trans_btc_cmd(u8 *cmd, u32 cmd_len)
{
	...
	addr = get_chip_remap(addr);
	if (addr < 0) 
		return;
	else if (addr < 8) {
		chip_id = chip_id_remap[addr][1];
...
static void trans_ltc_init_nonce(void)
{
	...
	for(sub=0;sub<Max_Chip_Num;sub++)
	{
		//chip_id = chip_id_remap[sub][1];
		//if (chip_id == -1) return;
		ln.header[2]=(LTC|sub);
		//ln.header[2]=(LTC|chip_id);
		ln.header[3]=0x23;
		...
		/* for the chip 15, because of the addr = 0xf, need special process */
#if defined(USE_STM3210E_EVAL)		
		else if (ltc_com_sel == 0) {
			ln.header[2]=(LTC|0x7);
			trans_buf_to_uart(BTC_COM2, (u8 *)&ln, sizeof(LTC_NONCE_INIT));
		}
		else {
			ln.header[2]=(LTC|0x7);
			trans_buf_to_uart(LTC_COM2, (u8 *)&ln, sizeof(LTC_NONCE_INIT));
		}
#endif		
#if 0		
#if defined(USE_STM3210E_EVAL)
		/* to support chip num > 8, need split trans to two uart channel */
		if (sub < 8) {
			if (ltc_com_sel)
				trans_buf_to_uart(BTC_COM1, (u8 *)&ln, sizeof(LTC_NONCE_INIT));
			else	
				trans_buf_to_uart(LTC_COM1, (u8 *)&ln, sizeof(LTC_NONCE_INIT));
		}
		else
		{
			if (ltc_com_sel)		
...
static void trans_ltc_cmd(u8 *cmd, u32 cmd_len)
{
	...
	if (ltc_com_sel == 0) {
		...
		{
			chip_id = chip_id_remap[cmd[2]&0xf][1];
			if (chip_id == 0xff) return;
			cmd[2] = LTC | chip_id;
			trans_buf_to_uart(LTC_COM1, cmd, cmd_len);
		}
...
static void process_btc_cmd(u8 *cmd, u32 cmd_len)
{
	...
	{
#ifdef FIXED_INIT_NONCE
		if (cmd_len == 8 && cmd[3] == 0) {
			if (cmd[2] == 0)
				trans_init_btc_nonce();
		}
		else
